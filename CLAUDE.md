...
- When a VMM returns an opaque error, read *its* log before theorising: `helper.log` is the VMM's own stderr. Two multi-cycle debugging spirals here (Cloud Hypervisor disabling sector-0 writes without `image_type=raw`; the VZ restore failure) would have been one step with the log in hand.
