...
- **Snapshot bundles** (`bundle.rs`, PRD §7 "share it with a colleague"): `sandbox.export` tars `vmstate` + `spec.json` + `machine-id` + `overlay.img` + `workspace` with a `manifest.json`; `sandbox.import` mints a *new* id and unpacks into its dir. Three refusals are load-bearing and must stay: **arch**, **backend**, and **protocol version** mismatches are caught in `check_compatible` (a saved RAM image crosses none of them, and the raw hypervisor error is unreadable); and a mount the importer doesn't have **refuses** rather than being created or dropped — a bundle must never name a path on someone else's machine and have it appear inside the VM, which is why `--remap` is the only way to satisfy it. Import also re-applies the *local* fleet policy, so a bundle can't carry permissions past the receiving machine's rules; if policy would alter the mounts it refuses, because the saved guest expects its exact device set. The macOS `machine-id` must travel or VZ rejects the restore.
- **Live permissions** (`live.rs`, PRD §4.5): `LivePermissions` holds net + auto-kill behind `RwLock`s; the proxy re-reads the policy **per connection** and the monitor **per tick**, which is what makes an edit bind an already-running agent. `registry.set_permissions` re-applies fleet policy — it's a second door into the same decision `run_sandbox` guards, so skipping it would let a user start compliant and then widen. **Mounts and CPU/RAM/disk cannot change** (device set fixed at creation) and the RPC refuses them explicitly rather than accepting and not enforcing. Offline sandboxes have no proxy bound at all, so they can be tightened *to* offline but never widened *from* it — deliberate: the strongest mode shouldn't be reversible at runtime.
...
- **Metrics** (`metrics.rs`, PRD §8): append-only `~/.agentos/metrics.jsonl` + `agentos metrics`. **Nothing is ever transmitted** — a security product that phones home has undermined its own pitch — so "adoption" reports *this* install (random local id, active days, run counts) and there is no server to add. "Zero-escape rate" is deliberately *not* claimed as a measurement: an escape is a hypervisor compromise the host can't observe, so the report says so and counts what is real instead (egress refused, and how much of it aimed at local/LAN space). Recording is best-effort everywhere: metrics must never be able to fail a run.
