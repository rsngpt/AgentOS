...
- **Snapshots outlive the daemon.** The registry is in memory, so the spec a restore needs would vanish with `agentosd` — the state file and overlay would still be on disk but unusable, which defeats parking work to come back to. The (policy-applied, canonicalised) spec is therefore written to `spec.json` beside the state, and on startup the daemon re-registers any sandbox directory holding both as `Snapshotted`. `ps` lists them and `restore` works across a daemon restart.
...
- **Restore (`agentos restore`) — works.** The guest resumes mid-execution: a counter loop snapshotted at `t1` continues at `t2`, and a second snapshot/restore of the same sandbox continues again at `t11` — no lost or repeated output across two full cycles.

  The bug that blocked this was **machine identity**. `VZGenericPlatformConfiguration.machineIdentifier` is regenerated at random on every launch unless pinned, and the header states plainly that a restore's identifier "must match the `machineIdentifier` of the saved virtual machine" — otherwise `VZErrorRestore` with an "invalid argument" reason. It explained every observation: our `vmconfig.json` really was byte-identical (the identifier was never *in* it), `validateSaveRestoreSupport` really did pass (it checks device types, not identity), and the failure really was independent of disks and of the live vsock connection. The vmhelper now persists the identifier as `machine-id` in the sandbox dir on first boot and reuses it forever after. Two related fixes: `saveMachineStateTo` refuses to overwrite, so a re-snapshot clears the old state file first; and `helper.log` is opened append-only, because a sandbox now outlives several helper processes and truncating discarded the diagnostics from the run that failed.
